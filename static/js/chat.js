// Chat-specific functionality for DineDesk
class ChatManager {
    ...
    setupSocketListeners() {
        const socket = this.dineDesk.socket;
        ...
        socket.on('bot_response_start', (data) => {
            this.startBotResponse(data.message_id);
        });
        
        socket.on('bot_response_stream', (data) => {
            this.updateStreamingResponse(data);
        });
        
        socket.on('bot_response_complete', (message) => {
            this.completeBotResponse(message);
        });
    }
    ...
    updateStreamingResponse(data) {
        const messageElement = this.streamingMessages.get(data.message_id);
    ...
    completeBotResponse(message) {
        const messageElement = this.streamingMessages.get(message.id);
        if (!messageElement) return;
        ...
        const newMessageElement = this.createMessageElement(message, false);
    ...
    handleQuickReply(text) {
        this.dineDesk.socket.emit('quick_reply', { text: text });
    }
    ...
    handleCardAction(action) {
        ...
        if (action.startsWith('book_')) {
            ...
            // Treat as a message
            this.dineDesk.socket.emit('send_message', { message: action });
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Wait for DineDesk to be initialized
    const initChatManager = () => {
        if (window.dineDesk && window.dineDesk.socket) {
