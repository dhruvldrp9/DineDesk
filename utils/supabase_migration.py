...
def migrate_to_supabase():
    """Migrate existing data to Supabase and set up tables"""
    try:
        # First, create the restaurants data directly in Supabase
        restaurants_data = [
            {
        ...
        # Check if restaurants already exist in Supabase
        existing_restaurants = supabase.table('restaurants').select('*').execute()
        
        if len(existing_restaurants.data) == 0:
            ...
            logging.info(f"Migrated {len(restaurants_data)} restaurants to Supabase")
        else:
            logging.info("Restaurants already exist in Supabase")
            
    except Exception as e:
        logging.error(f"Supabase migration error: {e}")
